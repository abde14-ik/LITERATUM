    ...
    return (
        <section id="inkwell" className="scroll-mt-32 py-6 lg:py-8">
                ...
                <div className="rounded-3xl border border-leather/70 bg-espresso/90 px-6 py-6 sm:px-8 sm:py-7 shadow-[0_24px_70px_rgba(0,0,0,0.9)]">
                    <div className="grid gap-8 md:grid-cols-[minmax(0,1.05fr)_minmax(0,1.1fr)] md:items-start">
                        ...
                        <div className="space-y-5">
                            ...
                            {latestEntry && (
                                <div
                                    className="mt-2 rounded-2xl border border-parchment/35 bg-espresso/70 px-4 py-4 shadow-inner shadow-black/40 cursor-pointer hover:bg-white/5 transition-colors"
                                    onClick={() => setReadingEntry(latestEntry)}
                                >
                                    ...
                                    <p className="mt-3 text-sm sm:text-[0.95rem] leading-relaxed text-parchment/85">
                                        {getPreviewText(latestEntry.text)}
                                    </p>
                                    <p className="mt-2 font-serif text-sm font-semibold text-gold">
                                        — {latestEntry.author}
                                    </p>
                                    {isTruncated(latestEntry.text) && (
                                        <p className="mt-1 text-[0.65rem] font-mono uppercase tracking-[0.18em] text-parchment/60 text-right">
                                            Read more »
                                        </p>
                                    )}
                                </div>
                            ...
                            {showArchive && archiveEntries.length > 0 && (
                                <div className="mt-3 max-h-64 space-y-3 overflow-y-auto pr-1">
                                    {archiveEntries.map((entry) => (
                                        <article
                                            key={entry.id}
                                            className="rounded-2xl border border-parchment/25 bg-espresso/80 px-4 py-3 shadow-inner shadow-black/40 cursor-pointer hover:bg-white/5 transition-colors"
                                        ...
                                        >
                                            ...
                                            <p className="mt-2 text-sm sm:text-[0.95rem] leading-relaxed text-parchment/85">
                                                {getPreviewText(entry.text)}
                                            </p>
                                            {isTruncated(entry.text) && (
                                                <p className="mt-1 text-[0.65rem] font-mono uppercase tracking-[0.18em] text-parchment/60 text-right">
                                                    Read more »
                                                </p>
                                            )}
            ...
            {readingEntry && (
                ...
                >
                    <article
                        className="bg-[#F2E8C9] text-[#231709] w-full max-w-2xl rounded-sm shadow-2xl border border-[#C5A059] relative max-h-[85vh] flex flex-col"
                        onClick={(event) => event.stopPropagation()}
                    >
                        <header className="sticky top-0 border-b border-[#d6c6a0] bg-[#F2E8C9] px-8 py-4 pr-12">
